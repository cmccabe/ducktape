...
class CheckScheduler(object):
    ...
    def check_non_empty_cluster_too_small(self):
        ...
        assert self.cluster.available().size() == 0
    ...
    def check_with_changing_cluster_availability(self):
        ...
        slots = self.cluster.alloc(Service.setup_cluster_spec(num_nodes=60))
        assert self.cluster.available().size() == 40
        t = scheduler.next()
        assert t.test_id == 0
        assert scheduler.peek() is None
        ...
        assert self.cluster.available().size() == 50
        t = scheduler.next()
        assert t.test_id == 1
        assert scheduler.peek() is None
        ...
        self.cluster.free(return_slots)
        assert self.cluster.available().size() == len(self.cluster)
        t = scheduler.next()
        assert t.test_id == 2
